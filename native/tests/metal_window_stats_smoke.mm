...
siliconrt::MetalWindowStatsResult expected_stats(
    const std::array<std::uint8_t, 16>& values) {
  siliconrt::MetalWindowStatsResult out = {0, 0, 0, 0};
  for (std::uint64_t i = 0; i < values.size(); ++i) {
    const std::uint64_t value = values[static_cast<std::size_t>(i)];
    const std::uint64_t w0 = (i & 31ull) + 1ull;
    const std::uint64_t w1 = ((i >> 3) & 15ull) + 3ull;
    out[0] += value;
    out[1] += value * value;
    out[2] += value * w0;
    out[3] += value * value * w1;
  }
  return out;
}
...
int main() {
  ...
  siliconrt_prefix_descriptor_t source_desc = {
      ...
      .prefix_hash_hex = "window-stats-src",
  ...
  const std::array<std::uint8_t, 16> expected_values = {
      4, 5, 6, 7, 8, 9, 10, 11, 16, 17, 18, 19, 20, 21, 22, 23};
  assert(result == expected_stats(expected_values));
  return 0;
}
