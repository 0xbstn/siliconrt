...
int main() {
  ...
  const auto prefix = builder.make_prefix("metal-store", 8192);
  ...
  assert(!backend.device_name().empty());
  ...
  assert(backend.has_store(stores.raw.sequence.backing_store_id));
  assert(backend.has_store(stores.raw.constant_state.backing_store_id));
  ...
  std::fill(prefix_sequence.bytes.begin(), prefix_sequence.bytes.end(), 0x5A);
  std::fill(prefix_constant.bytes.begin(), prefix_constant.bytes.end(), 0x7B);
  ...
  assert(borrowed_sequence.bytes.front() == 0x5A);
  assert(borrowed_constant.bytes.front() == 0x7B);
  ...
  assert(prefix_sequence.bytes.front() == 0x5A);
  assert(promoted_sequence.bytes.front() == 0x33);
  assert(promoted_constant.bytes.front() == 0x7B);
